...
agent = Agent(manager, [SearchObjectsTool(), PickObjectTool(), ChangeTaskTool()], system_prompt, max_iterations=5)

# easy task
print(agent.run("how many tables has the robot seen so far?", [system_action_log]))

# medium task
print(agent.run("can you pick up an apple?", [system_action_log]))
